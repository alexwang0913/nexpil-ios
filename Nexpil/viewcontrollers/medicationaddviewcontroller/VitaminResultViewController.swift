...
class VitaminResultViewController: UIViewController {
    ...
    @objc func gotoNext1(sender : UITapGestureRecognizer) {
        ...
        let data = MyMedication.init(prescribe: DataUtils.getPrescribed()!, directions: "", dose: DataUtils.getMedicationDose()!, image: "", quantity: 0, type: "", taketime: "", medicationname: DataUtils.getMedicationName()!, filedDate: "", warning: "", frequency: DataUtils.getMedicationFrequency()!, strength: "", pharmacy: "", patientname: "", lefttablet: "", prescription: DataUtils.getPrescription(), createat: currentDate + " \(hour):\(min1)", endat:"", id:Int(DataUtils.getDrugId()!)!, amount: "", asneeded: 0)
        datas.append(data)
