...
//    static let APIURL = "http://nextec.xyz/"
//    static let APIURL = "http://localhost/nexpil/"
    static let APIURL = "http://52.90.52.40/nexpil/"
    static let PROFILEURL = APIURL + "upload/profile/"
    static let PHOTOURL = APIURL + "upload/post/photo/"
    static let VIDEOURL = APIURL + "upload/post/video/"
    
    static let AUTH_URL = "patient_authorize.php"
    static let PRODUCT_URL = "drug_product.php"
    static let Pharmacy_URL = "pharmace.php"
    ...
    static let MEDICAL_CONDITION_URL = "medical_condition.php"
    static let DRUG_HISTORY_URL =  "drug_history.php"
    static let HEALTH_BLOODGLUCOSE_URL = "health_bloodglucose.php"
    
    static let PATIENT_URL = "patient.php"
    static let NOTIFICATION_URL = "notification.php"
    ...
    class func customActivityIndicatory(_ viewContainer: UIView, startAnimate:Bool? = true) {
        ...
        viewBackgroundLoading.backgroundColor = UIColor(red: 68/255.0, green: 68/255.0, blue: 68/255.0, alpha: 1.0) 
    ...
    class func getPharmacy(completionHandler:@escaping (_ Success:Bool,_ Data:[Pharmacy])->Void ){
       let params = [
                    ...
                    if result == "true" {
                        if let jsonData = json["data"] as? [[String:Any]]{
                            for i in 0..<jsonData.count{
                                let pharmacy = Pharmacy.init(json: jsonData[i])
                                returnData.append(pharmacy)
                             }
                        }
                        completionHandler(true,returnData)
                    }else {
                        completionHandler(false,returnData)
                    }
                }
            })
    }
    class func deleteMyPharmacy(address:String,completionHandler:@escaping (_ Success:Bool)->Void ){
        let params = [
            "Userid" : PreferenceHelper().getId(),
            "choice" : "5",
            "Address":address
    ...
    class func getCommunityUser(completionHandler:@escaping (_ Success:Bool,_ Data:[CommunityUser])->Void ){
        let params = [
            "userid" : PreferenceHelper().getId(),
            "choice" : "0"
            ] as [String : Any]
            ...
            .responseJSON(completionHandler: { response in                
                ...
                if let data = response.result.value {
                    ...
                    if result == "true" {
                        if let jsonData = json["data"] as? [[String:Any]]{
                            for i in 0..<jsonData.count{
                                let Community = CommunityUser.init(json: jsonData[i])
