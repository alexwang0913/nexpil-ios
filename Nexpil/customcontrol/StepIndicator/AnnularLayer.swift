...
//                    self.drawFullCircleAnimated()
    ...
    private func drawText() {
        ...
        self.centerTextLayer.string = self.label
        self.centerTextLayer.frame = self.bounds
        self.centerTextLayer.position = CGPoint(x: self.bounds.midX, y: self.bounds.maxY + 20)
        self.centerTextLayer.contentsScale = UIScreen.main.scale
        self.centerTextLayer.foregroundColor = isCurrent ? UIColor.black.cgColor : self.annularDefaultColor?.cgColor
//        self.centerTextLayer.alignmentMode = CATextLayerAlignmentMode.center
        self.centerTextLayer.alignmentMode = "center"
        let fontSize = sideLength * 0.3
...
//        self.fullCircleLayer.borderWidth = 1
//        self.fullCircleLayer.borderColor = self.tintColor?.cgColor
    ...
    private func drawFullCircleBorder() {
        let fullCirclePath = UIBezierPath()
        let sideLength = fmin(self.frame.width, self.frame.height)
        let circlesRadius = sideLength / 2.0
