...
//            }
            ...
            self.containerLayer.addSublayer(textLayer)
        ...
        self.horizontalLineLayers[0].isFinished = true
    }
...
//        let stepWidth = self.numberOfSteps == 1 ? 0 : (self.containerLayer.frame.width - self.lineMargin * 2 - diameter) / CGFloat(self.numberOfSteps + 1)
        let stepWidth = (self.containerLayer.frame.width - diameter * CGFloat(self.numberOfSteps)) / CGFloat(self.numberOfSteps + 1)
        
        let y = self.containerLayer.frame.height / 2.0
        
        let labels = ["mg/dl", "Time", "Date"]
        for i in 0..<self.annularLayers.count {
            let annularLayer = self.annularLayers[i]
            var x = self.numberOfSteps == 1 ? self.containerLayer.frame.width / 2.0 - self.circleRadius : self.lineMargin + CGFloat(i) * (stepWidth + diameter)

            annularLayer.label = labels[i]
            annularLayer.frame = CGRect(x: CGFloat(i+1) * stepWidth + diameter * CGFloat(i), y: y - self.circleRadius, width: diameter, height: diameter)
...
//            if (i < self.numberOfSteps - 1) {
                ...
                lineLayer.frame = CGRect(x: x, y: y - 1, width: stepWidth, height: 3)
                self.applyLineStyle(lineLayer: lineLayer)
                lineLayer.updateStatus()
//            }
        }
        
        if self.horizontalLineLayers.count > 2 {
            // First line
            let lineLayer = self.horizontalLineLayers[3]
            lineLayer.frame = CGRect(x: 3 * (stepWidth + diameter), y: y-1, width: stepWidth, height: 3)
            self.applyLineStyle(lineLayer: lineLayer)
            lineLayer.updateStatus()
        }
    }
    ...
    private func setCurrentStep(step:Int) {
        for i in 0..<self.numberOfSteps {
            if i < step {
                ...
                self.setLineFinished(isFinished: true, index: i)
            }
            else if i == step {
                ...
                self.setLineFinished(isFinished: false, index: i)
            }
        }
    }
